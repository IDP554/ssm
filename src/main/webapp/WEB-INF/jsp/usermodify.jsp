...
<%@include file="common/head.jsp"%>
    <div class="right">
        ...
        <form id="userForm" name="userForm" method="post" action="/modify.html">
		       ...
		       <div>
				...
				<div>
					...
					<select name="userRole" id="userRole">
                            <option value="1" <c:if test="${user.userRole ==1 }">selected</c:if>>管理员</option>
                            <option value="2" <c:if test="${user.userRole ==2 }">selected</c:if>>经理</option>
                            <option value="3" <c:if test="${user.userRole ==3 }">selected</c:if>>普通员工</option>
                    </select>
        			<font color="red"></font>
                </div>
